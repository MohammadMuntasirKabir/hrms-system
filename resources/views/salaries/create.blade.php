<x-layouts::app :title="__('Add Salary')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <flux:heading size="xl" class="text-zinc-900 dark:text-white">{{ __('Add Salary') }}</flux:heading>
                <flux:subheading class="text-zinc-500 dark:text-zinc-400">{{ __('Set up employee compensation') }}</flux:subheading>
            </div>
        </div>
        ...
        <div class="hrms-form-card max-w-2xl">
            <form method="POST" action="{{ route('salaries.store') }}" class="flex flex-col gap-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <flux:select name="pay_frequency" :label="__('Pay Frequency')" required>
                        <option value="monthly" @selected(old('pay_frequency') == 'monthly')>{{ __('Monthly') }}</option>
                        <option value="bi_weekly" @selected(old('pay_frequency') == 'bi_weekly')>{{ __('Bi-Weekly') }}</option>
                        <option value="weekly" @selected(old('pay_frequency') == 'weekly')>{{ __('Weekly') }}</option>
                    </flux:select>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <flux:input name="base_salary" :label="__('Base Salary')" type="number" step="0.01" :value="old('base_salary')" required placeholder="0.00" />
                    <flux:input name="allowances" :label="__('Allowances')" type="number" step="0.01" :value="old('allowances', 0)" placeholder="0.00" />
                    <flux:input name="deductions" :label="__('Deductions')" type="number" step="0.01" :value="old('deductions', 0)" placeholder="0.00" />
                </div>
                ...
                <div class="rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 p-4">
                    <flux:text class="text-sm text-blue-700 dark:text-blue-300">
                        <strong>{{ __('Net Salary') }}:</strong> {{ __('Base') }} + {{ __('Allowances') }} - {{ __('Deductions') }}
                    </flux:text>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
                    <flux:input name="currency" :label="__('Currency')" :value="old('currency', 'BDT')" placeholder="BDT" />
                    <flux:input name="effective_from" :label="__('Effective From')" type="date" :value="old('effective_from')" required />
                    <flux:input name="effective_until" :label="__('Effective Until')" type="date" :value="old('effective_until')" hint="{{ __('Leave empty for ongoing') }}" />
                </div>
                ...
                <flux:select name="status" :label="__('Status')" required>
                    ...
                    <option value="revised" @selected(old('status') == 'revised')>{{ __('Revised') }}</option>
                </flux:select>

                <flux:textarea name="notes" :label="__('Notes')" :value="old('notes')" placeholder="Salary notes..." rows="2" />

                <div class="flex items-center justify-end gap-3 pt-2">
                    <flux:button :href="route('salaries.index')" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
                    <flux:button type="submit" variant="primary" icon="plus">{{ __('Create Salary') }}</flux:button>
                </div>
            </form>
        </div>
    </div>
