<x-layouts::app :title="__('Edit Department')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <flux:heading size="xl" class="text-zinc-900 dark:text-white">{{ __('Edit Department') }}</flux:heading>
                <flux:subheading class="text-zinc-500 dark:text-zinc-400">{{ __('Update department information') }}</flux:subheading>
            </div>
        </div>
        ...
        <div class="hrms-form-card max-w-2xl">
            <form method="POST" action="{{ route('departments.update', $department) }}" class="flex flex-col gap-6">
                @csrf @method('PUT')

                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <flux:input name="name" :label="__('Department Name')" :value="old('name', $department->name)" required />
                    <flux:input name="code" :label="__('Code')" :value="old('code', $department->code)" maxlength="20" />
                </div>

                <flux:textarea name="description" :label="__('Description')" :value="old('description', $department->description)" rows="3" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <flux:select name="parent_department_id" :label="__('Parent Department')">
                        ...
                        @foreach ($parentDepartments as $parent)
                            <option value="{{ $parent->id }}" @selected(old('parent_department_id', $department->parent_department_id) == $parent->id)>{{ $parent->name }}</option>
                        @endforeach
                    </flux:select>
                ...
                <flux:checkbox name="is_active" :label="__('Active Department')" :checked="old('is_active', $department->is_active)" />

                <div class="flex items-center justify-end gap-3 pt-2">
                    <flux:button :href="route('departments.index')" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
                    <flux:button type="submit" variant="primary" icon="check">{{ __('Update Department') }}</flux:button>
                </div>
            </form>
        </div>
    </div>
