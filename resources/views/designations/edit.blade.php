<x-layouts::app :title="__('Edit Designation')">
    <div class="flex h-full w-full flex-1 flex-col gap-6 p-4 lg:p-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                <flux:heading size="xl" class="text-zinc-900 dark:text-white">{{ __('Edit Designation') }}</flux:heading>
                <flux:subheading class="text-zinc-500 dark:text-zinc-400">{{ __('Update job title and hierarchy level') }}</flux:subheading>
            </div>
        </div>
        ...
        <div class="hrms-form-card max-w-2xl">
            <form method="POST" action="{{ route('designations.update', $designation) }}" class="flex flex-col gap-6">
                @csrf @method('PUT')

                <flux:input name="title" :label="__('Designation Title')" :value="old('title', $designation->title)" required />

                <flux:textarea name="description" :label="__('Description')" :value="old('description', $designation->description)" rows="3" />
                    ...
                    @if ($companies->count() > 1)
                        <flux:select name="company_id" :label="__('Company')" required>
                            ...
                            @foreach ($companies as $company)
                                <option value="{{ $company->id }}" @selected(old('company_id', $designation->company_id) == $company->id)>{{ $company->name }}</option>
                            @endforeach
                        </flux:select>
                    @else
                ...
                <flux:select name="level" :label="__('Hierarchy Level')" required>
                    ...
                    @for ($i = 0; $i <= 10; $i++)
                        ...
                        <option value="{{ $i }}" @selected(old('level', $designation->level) == $i)>{{ $i }} — {{ $labels[$i] }}</option>
                    @endfor
                </flux:select>

                <flux:checkbox name="is_active" :label="__('Active Designation')" :checked="old('is_active', $designation->is_active)" />

                <div class="flex items-center justify-end gap-3 pt-2">
                    <flux:button :href="route('designations.index')" variant="ghost" wire:navigate>{{ __('Cancel') }}</flux:button>
                    <flux:button type="submit" variant="primary" icon="check">{{ __('Update Designation') }}</flux:button>
                </div>
            </form>
        </div>
    </div>
