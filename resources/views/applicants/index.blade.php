<x-layouts::app :title="__('Applicants')">
    <div class="flex h-full w-full flex-1 flex-col gap-4 p-3 sm:gap-6 sm:p-4 lg:p-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
            <div>
                <flux:heading size="lg" class="text-zinc-900 dark:text-white lg:text-xl">{{ __('Job Applicants') }}</flux:heading>
                <flux:subheading class="text-zinc-500 dark:text-zinc-400 text-sm">{{ __('Review and manage job applicants') }}</flux:subheading>
            </div>
            @can('applicants.create')
                <flux:button :href="route('applicants.create')" variant="primary" icon="plus" size="sm">{{ __('Add Applicant') }}</flux:button>
            @endcan
        </div>

        <!-- Status Stats -->
        <div class="flex flex-wrap gap-2 sm:gap-3">
            <div class="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800">
                <span class="text-xs font-medium text-zinc-500 dark:text-zinc-400">{{ __('Pending') }}</span>
                <span class="text-xs sm:text-sm font-bold text-zinc-700 dark:text-zinc-300">{{ $statusCounts['pending'] }}</span>
            </div>
            <div class="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                <span class="text-xs font-medium text-blue-600 dark:text-blue-400">{{ __('Reviewing') }}</span>
                <span class="text-xs sm:text-sm font-bold text-blue-700 dark:text-blue-300">{{ $statusCounts['reviewing'] }}</span>
            </div>
            <div class="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-purple-50 dark:bg-purple-900/20">
                <span class="text-xs font-medium text-purple-600 dark:text-purple-400">{{ __('Shortlisted') }}</span>
                <span class="text-xs sm:text-sm font-bold text-purple-700 dark:text-purple-300">{{ $statusCounts['shortlisted'] }}</span>
            </div>
            <div class="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-emerald-50 dark:bg-emerald-900/20">
                <span class="text-xs font-medium text-emerald-600 dark:text-emerald-400">{{ __('Hired') }}</span>
                <span class="text-xs sm:text-sm font-bold text-emerald-700 dark:text-emerald-300">{{ $statusCounts['hired'] }}</span>
            </div>
            <div class="flex items-center gap-1.5 px-2 py-1 sm:px-3 sm:py-1.5 rounded-lg bg-red-50 dark:bg-red-900/20">
                <span class="text-xs font-medium text-red-600 dark:text-red-400">{{ __('Rejected') }}</span>
                <span class="text-xs sm:text-sm font-bold text-red-700 dark:text-red-300">{{ $statusCounts['rejected'] }}</span>
            </div>
        </div>
        ...
        <div class="flex flex-wrap gap-1.5 sm:gap-2">
            ...
            @if($currentUser->isSuperAdmin() && $companies->count() > 1)
                <span class="w-px h-6 bg-zinc-200 dark:bg-zinc-700 mx-1 self-center hidden sm:block"></span>
        ...
        @if (session('status'))
            <div class="rounded-lg bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800 p-3 sm:p-4 flex items-center gap-3">
        ...
        @endif
        @if (session('error'))
            <div class="rounded-lg bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 p-3 sm:p-4 flex items-center gap-3">
            ...
            <div class="overflow-x-auto">
                <flux:table>
                    <flux:table.columns>
                        ...
                        <flux:table.column class="hidden md:table-cell">{{ __('Position') }}</flux:table.column>
                        @if ($currentUser->isSuperAdmin())
                            <flux:table.column class="hidden lg:table-cell">{{ __('Company') }}</flux:table.column>
                        @endif
                        <flux:table.column class="hidden sm:table-cell">{{ __('Expected Salary') }}</flux:table.column>
                        <flux:table.column class="hidden md:table-cell">{{ __('Applied') }}</flux:table.column>
                            ...
                            <flux:table.row>
                                <flux:table.cell>
                                    <div class="flex items-center gap-2 sm:gap-3">
                                        <flux:avatar :name="$applicant->full_name" size="xs" class="sm:hidden" />
                                        <flux:avatar :name="$applicant->full_name" size="sm" class="hidden sm:flex" />
                                        <div class="min-w-0">
                                            <a href="{{ route('applicants.show', $applicant) }}" class="font-medium text-hrms-600 dark:text-hrms-400 hover:underline truncate block" wire:navigate>{{ $applicant->full_name }}</a>
                                            <div class="text-xs text-zinc-500 dark:text-zinc-400 truncate">{{ $applicant->email }}</div>
                                            <!-- Show position on mobile -->
                                            <div class="text-xs text-zinc-400 md:hidden">
                                                @if ($applicant->designation)
                                                    {{ $applicant->designation->title }}
                                                @else
                                                    —
                                                @endif
                                            </div>
                                ...
                                @if ($currentUser->isSuperAdmin())
                                    <flux:table.cell class="hidden lg:table-cell text-zinc-500 dark:text-zinc-400">{{ $applicant->company?->name ?? '—' }}</flux:table.cell>
                                @endif
                                ...
                                <flux:table.cell class="hidden md:table-cell text-sm text-zinc-500 dark:text-zinc-400">{{ $applicant->created_at->format('M d, Y') }}</flux:table.cell>
                                <flux:table.cell>
                                    @php
                                        ...
                                        @elseif ($applicant->isHired())
                                            @if ($applicant->updated_at->gte(now()->subDay()))
                                                <form method="POST" action="{{ route('applicants.undo-hire', $applicant) }}" class="inline">
                                                    @csrf
                                                    <flux:button type="submit" size="xs" variant="outline" icon="arrow-uturn-left" title="{{ __('Undo Hire') }}" onclick="return confirm('{{ __('Undo this hire? The employee record will be deleted.') }}')"></flux:button>
                                                </form>
                                            @endif
