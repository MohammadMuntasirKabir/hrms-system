        ...
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
            ...
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 sm:gap-3 flex-wrap">
                    <flux:heading size="lg" class="text-zinc-900 dark:text-white lg:text-xl truncate">{{ $applicant->full_name }}</flux:heading>
                    @php
                ...
                <flux:subheading class="text-zinc-500 dark:text-zinc-400 text-sm truncate">{{ $applicant->email }} · {{ $applicant->company?->name }}</flux:subheading>
            </div>
            <div class="hrms-actions shrink-0">
                @if (!$applicant->isHired() && !$applicant->isRejected() && auth()->user()->isSeniorMember())
                    <flux:button :href="route('applicants.edit', $applicant)" variant="outline" icon="pencil" wire:navigate size="sm">{{ __('Edit') }}</flux:button>
                @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="lg:col-span-2 space-y-4">
                <div class="hrms-card">
                    <div class="hrms-card-body">
                        <flux:heading size="md" class="mb-4 text-zinc-900 dark:text-white">{{ __('Applicant Information') }}</flux:heading>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-zinc-700 dark:text-zinc-300">{{ $applicant->expected_salary ? $applicant->currency . ' ' . number_format($applicant->expected_salary, 0) : '—' }}</p>
                            </div>
                            <div>
                                ...
                                <p class="mt-1 text-zinc-700 dark:text-zinc-300">{{ $applicant->available_from ? $applicant->available_from->format('M d, Y') : '—' }}</p>
                            </div>
                            <div>
                                <flux:text class="text-xs font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">{{ __('Source') }}</flux:text>
                                <p class="mt-1 text-zinc-700 dark:text-zinc-300">{{ $applicant->source ?? '—' }}</p>
                            </div>
                            <div>
                                <flux:text class="text-xs font-medium text-zinc-400 dark:text-zinc-500 uppercase tracking-wider">{{ __('Applied') }}</flux:text>
                                <p class="mt-1 text-zinc-700 dark:text-zinc-300">{{ $applicant->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
            ...
            <div class="space-y-4">
                @if (!$applicant->isHired() && !$applicant->isRejected() && auth()->user()->isSeniorMember())
                    <div class="hrms-card">
                        <div class="hrms-card-body">
                            <flux:heading size="sm" class="mb-3 text-zinc-900 dark:text-white">{{ __('Quick Actions') }}</flux:heading>
                            <div class="hrms-actions mb-4">
                                @if ($applicant->isPending())
                                    <form method="POST" action="{{ route('applicants.review', $applicant) }}" class="flex-1">
                                        @csrf
                                        <flux:button type="submit" variant="outline" icon="eye" class="w-full" size="sm">{{ __('Review') }}</flux:button>
                                    </form>
                                @endif
                                @if (in_array($applicant->status, ['pending', 'reviewing']))
                                    <form method="POST" action="{{ route('applicants.shortlist', $applicant) }}" class="flex-1">
                                        @csrf
                                        <flux:button type="submit" variant="primary" icon="star" class="w-full" size="sm">{{ __('Shortlist') }}</flux:button>
                                    </form>
                                @endif
                            </div>
                            ...
                            <form method="POST" action="{{ route('applicants.hire', $applicant) }}" class="flex flex-col gap-3">
                                @csrf
                                ...
                                <div class="grid grid-cols-2 gap-2">
                                    <flux:select name="contract_type" :label="__('Type')" required>
                                    ...
                                    <flux:select name="role" :label="__('Role')" required>
                                        ...
                                        <option value="department_head" @selected(old('role') == 'department_head')>{{ __('Dept Head') }}</option>
                                    </flux:select>
                                </div>
                                <div class="grid grid-cols-2 gap-2">
